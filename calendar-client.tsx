...
export default function CalendarClient() {
  ...
  const itemsByDate = useMemo(() => {
    ...
    for (const item of items) map.set(item.scheduledFor, [...(map.get(item.scheduledFor) ?? []), item]);
    return map;
  }, [items]);
  ...
  async function assignTask() {
    ...
    try {
      ...
      const { data, error: insertError } = await supabase
        .from("task_assignments")
        .insert({
          brief_id: selectedItem.briefId,
          assigned_to: selectedAssignee,
          assigned_by: currentUserId,
          due_date: selectedItem.scheduledFor,
          status: "todo",
          priority: "normal",
        })
        .select("id,brief_id,assigned_to,status,priority,due_date")
      ...
      setTasks((current) => [...current, data as TaskAssignment]);
      setSelectedAssignee("");
      setMessage("Task berhasil di-assign. Task sekarang muncul di Overview akun penerima.");
    } catch (err) {
  ...
  async function moveBrief(briefId: string, dateString: string) {
    ...
    try {
      const supabase = createClient();
      const { error: updateError } = await supabase.from("content_briefs").update({ scheduled_for: dateString }).eq("id", briefId);
      if (updateError) throw updateError;
      setItems((currentItems) => currentItems.map((item) => item.briefId === briefId ? { ...item, scheduledFor: dateString } : item));
      setTasks((currentTasks) => currentTasks.map((task) => task.brief_id === briefId ? { ...task, due_date: dateString } : task));
  ...
  function onCardDragStart(event: DragEvent<HTMLElement>, briefId: string) {
    setDraggingId(briefId); event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", briefId);
  }
  async function onDayDrop(event: DragEvent<HTMLElement>, dateString: string) {
    event.preventDefault(); const briefId = event.dataTransfer.getData("text/plain") || draggingId; if (briefId) await moveBrief(briefId, dateString);
  }
      ...
      <main className="mx-auto max-w-[1500px] px-4 py-8 lg:px-6 lg:py-10">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-500">Brief yang sudah dijadwalkan muncul di sini. Superadmin dapat meng-assign task langsung dari kartu calendar.</p>
          </div>
        ...
        <div className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
          <section className="min-w-0 rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-5">
              <div><h2 className="text-xl font-bold capitalize">{monthTitle(month)}</h2><p className="mt-1 text-xs text-slate-400">{monthItems.length} content terjadwal bulan ini</p></div>
              ...
              {calendarCells.map((cell, index) => {
                ...
                return <div key={cell.dateString} onDragOver={(event) => { event.preventDefault(); setDragOverDate(cell.dateString); }} onDragLeave={() => { if (dragOverDate === cell.dateString) setDragOverDate(null); }} onDrop={(event) => onDayDrop(event, cell.dateString)} className={`min-h-32 border-b border-r border-slate-100 p-2 transition md:min-h-40 ${dragOverDate === cell.dateString ? "bg-blue-50 ring-2 ring-inset ring-blue-300" : "bg-white"}`}>
                  ...
                  <div className="space-y-2">
                    {dayItems.map((item) => {
                      const itemTasks = tasks.filter((task) => task.brief_id === item.briefId);
                      return <article key={item.briefId} draggable onDragStart={(event) => onCardDragStart(event, item.briefId)} onDragEnd={() => { setDraggingId(null); setDragOverDate(null); }} onClick={() => chooseItem(item)} className={`cursor-grab rounded-xl border p-2.5 text-left shadow-sm transition ${draggingId === item.briefId ? "opacity-40" : selectedId === item.briefId ? "border-blue-400 bg-blue-50" : "border-slate-200 bg-white hover:border-blue-300"}`}>
                        <div className="flex items-center justify-between gap-2"><span className="truncate text-[10px] font-semibold uppercase tracking-wide text-blue-600">{item.brandName}</span>{movingId === item.briefId && <span className="text-[9px] text-slate-400">saving...</span>}</div>
                        <p className="mt-1 line-clamp-3 text-[11px] font-semibold leading-4 text-slate-800">{item.title}</p>
                        <div className="mt-2 flex flex-wrap gap-1"><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[9px] text-slate-500">{item.format || "content"}</span><span className={`rounded-full px-2 py-0.5 text-[9px] ${item.humanQcStatus === "approved" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}>{item.humanQcStatus === "approved" ? "QC ✓" : "QC ulang"}</span>{itemTasks.length > 0 && <span className="rounded-full bg-violet-50 px-2 py-0.5 text-[9px] font-semibold text-violet-600">{itemTasks.length} assigned</span>}</div>
                      </article>;
                    })}
                  </div>
                </div>;
          ...
          <aside className="space-y-4">
            <div className="rounded-3xl bg-slate-950 p-5 text-white"><p className="text-xs font-semibold uppercase tracking-widest text-blue-300">Task Assignment</p><h2 className="mt-2 text-xl font-bold">Assign pekerjaan langsung dari calendar.</h2><p className="mt-2 text-sm leading-6 text-slate-300">Klik card content, pilih anggota tim, lalu assign. Task otomatis masuk ke Overview akun penerima.</p></div>

            <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="font-semibold">Selected Content</h2>
              {!selectedItem ? <p className="mt-3 text-sm leading-6 text-slate-500">Klik kartu content untuk melihat detail, memindahkan tanggal, dan melakukan assignment.</p> : <div className="mt-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-blue-600">{selectedItem.brandName}</p><p className="mt-1 text-sm font-semibold leading-5">{selectedItem.title}</p><p className="mt-2 text-xs text-slate-400">Jadwal: {formatShortDate(selectedItem.scheduledFor)}</p>
