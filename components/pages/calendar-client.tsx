...
export default function CalendarClient() {
  ...
  useEffect(() => {
    ...
    if (visibleItems.some((item) => item.briefId === selectedId)) return;
    setSelectedId(null);
    setQuickMoveDate("");
    setDesignFileUrl("");
    setMessage("");
  ...
  const selectedItem = visibleItems.find((item) => item.briefId === selectedId) ?? null;
    ...
    try {
      ...
      setItems((currentItems) =>
        currentItems.map((current) =>
          current.briefId === item.briefId ? { ...current, designStatus: nextStatus } : current,
        ),
      );
  ...
  async function saveDesignFile(item: CalendarItem) {
    ...
    try {
      ...
      setItems((currentItems) =>
        currentItems.map((current) =>
          current.briefId === item.briefId ? { ...current, designFileUrl: value || null } : current,
        ),
      );
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="inline-flex rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-600">
                {activeBrandHydrated ? contextLabel : "Loading brand context..."}
              </div>
            </div>
            <h1 className="mt-3 text-3xl font-bold tracking-tight md:text-4xl">Content Calendar</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-500">
              Calendar otomatis mengikuti Active Brand. Drag kartu ke tanggal lain untuk mengubah jadwal tanpa membuka brief satu per satu.
            </p>
          </div>
        ...
        <div className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
          <section className="min-w-0 rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-5">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-400">{monthItems.length} content terjadwal · {contextLabel}</p>
              </div>
            ...
            <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-50">
              {weekdayLabels.map((label) => (
                <div key={label} className="px-2 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-400">{label}</div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-7">
              {calendarCells.map((cell, index) => {
                if (!cell) {
                  return <div key={`blank-${index}`} className="min-h-32 border-b border-r border-slate-100 bg-slate-50/40 md:min-h-40" />;
                }
                ...
                return (
                  <div
                    ...
                    className={`min-h-32 border-b border-r border-slate-100 p-2 transition md:min-h-40 ${dragOverDate === cell.dateString ? "bg-blue-50 ring-2 ring-inset ring-blue-300" : "bg-white"}`}
                  >
                    <div className="mb-2 flex items-center justify-between">
                      <span className={`grid h-7 w-7 place-items-center rounded-full text-xs font-semibold ${today ? "bg-blue-600 text-white" : "text-slate-500"}`}>{cell.dayNumber}</span>
                      {dayItems.length > 0 && <span className="text-[10px] font-semibold text-slate-300">{dayItems.length}</span>}
                    </div>
                    ...
                    <div className="space-y-2">
                      {dayItems.map((item) => (
                        <article
                          ...
                          className={`cursor-grab rounded-xl border p-2.5 text-left shadow-sm transition ${draggingId === item.briefId ? "opacity-40" : selectedId === item.briefId ? "border-blue-400 bg-blue-50" : "border-slate-200 bg-white hover:border-blue-300"}`}
                        >
                          <div className="flex items-center justify-between gap-2">
                            <span className="truncate text-[10px] font-semibold uppercase tracking-wide text-blue-600">{item.brandName}</span>
                            {movingId === item.briefId && <span className="text-[9px] text-slate-400">saving...</span>}
                          </div>
                          <p className="mt-1 line-clamp-3 text-[11px] font-semibold leading-4 text-slate-800">{item.title}</p>
                          <div className="mt-2 flex flex-wrap gap-1">
                            <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[9px] text-slate-500">{item.format || "content"}</span>
                            <span className={`rounded-full px-2 py-0.5 text-[9px] ${item.humanQcStatus === "approved" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}>
                              {item.humanQcStatus === "approved" ? "QC ✓" : "QC ulang"}
                            </span>
                            <span className={`rounded-full px-2 py-0.5 text-[9px] font-semibold ${item.designStatus === "designed" ? "bg-violet-50 text-violet-700" : "bg-blue-50 text-blue-700"}`}>
                              {item.designStatus === "designed" ? "Designed ✓" : "Ready to Design"}
                            </span>
          ...
          <aside className="space-y-4">
            <div className="rounded-3xl bg-slate-950 p-5 text-white">
              <p className="text-xs font-semibold uppercase tracking-widest text-blue-300">Active Context</p>
              <h2 className="mt-2 text-xl font-bold">{contextLabel}</h2>
              <p className="mt-2 text-sm leading-6 text-slate-300">
                {activeBrand.id === ACTIVE_BRAND_ALL
                  ? "Semua scheduled content ditampilkan. Pilih satu brand di sidebar untuk fokus pada klien tertentu."
                  : "Hanya scheduled content milik brand aktif yang ditampilkan. Perubahan jadwal tetap tersimpan langsung ke Supabase."}
              </p>
            </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              {!selectedItem ? (
                <p className="mt-3 text-sm leading-6 text-slate-500">Klik salah satu kartu content untuk memindahkan ke tanggal lain, termasuk bulan yang berbeda.</p>
              ) : (
                <div className="mt-4">
                  <p className="text-xs font-semibold uppercase tracking-wider text-blue-600">{selectedItem.brandName}</p>
                  ...
                  <div className="mt-5 border-t border-slate-200 pt-5">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Design Status</p>
                        <p className={`mt-1 text-sm font-bold ${selectedItem.designStatus === "designed" ? "text-violet-700" : "text-blue-700"}`}>
                          {selectedItem.designStatus === "designed" ? "Designed ✓" : "Ready to Design"}
                        </p>
                      </div>
                      {selectedItem.designStatus === "designed" ? (
                        <button onClick={() => setDesignStatus(selectedItem, "ready_to_design")} disabled={designSaving} className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700 hover:bg-blue-100 disabled:opacity-50">Set Ready</button>
                      ) : (
                        <button onClick={() => setDesignStatus(selectedItem, "designed")} disabled={designSaving} className="rounded-xl bg-violet-600 px-3 py-2 text-xs font-semibold text-white hover:bg-violet-700 disabled:opacity-50">Mark Designed</button>
                      )}
                    </div>
                    ...
                    />
                    <button onClick={() => saveDesignFile(selectedItem)} disabled={designSaving} className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold hover:bg-slate-50 disabled:opacity-50">
                      {designSaving ? "Menyimpan..." : "Simpan Link Design"}
                    </button>
                    {selectedItem.designFileUrl && (
                      <a href={selectedItem.designFileUrl} target="_blank" rel="noreferrer" className="mt-2 block w-full rounded-2xl bg-slate-950 px-4 py-3 text-center text-sm font-semibold text-white hover:bg-slate-800">Buka File Design ↗</a>
                    )}
                  </div>

                  <button onClick={() => router.push(`/brief/${selectedItem.briefId}`)} className="mt-4 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold hover:bg-slate-50">Buka Full Brief →</button>
                </div>
              )}
            </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="text-xs text-slate-400">{visibleItems.length} scheduled</span>
              </div>
