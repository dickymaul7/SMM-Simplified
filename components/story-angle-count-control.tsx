...
export default function StoryAngleCountControl() {
  ...
  useEffect(() => {
    ...
    const interceptedFetch: typeof window.fetch = async (input, init) => {
      ...
      if (!originalUrl.includes("/api/ai/angles")) {
      ...
      if (typeof nextInit.body === "string") {
        try {
          ...
          // Leave malformed/non-JSON requests untouched.
        }
      }
  ...
  function updateGenerateButtonLabel(value: number) {
    const button = Array.from(document.querySelectorAll("button")).find((item) =>
      item.textContent?.includes(BUTTON_TEXT),
    );
    if (!button) return;
    button.textContent = `Cari Kasus & Buat ${value} Story Angles →`;
  }
  ...
  function selectCount(value: number) {
    ...
    try {
      ...
      // The in-memory value still works when storage is unavailable.
    }

    updateGenerateButtonLabel(value);
  }
