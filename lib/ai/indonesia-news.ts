...
export function buildIndonesiaNewsQueries(input: {
  ...
  return [
    `${topic} Indonesia berita terbaru tren bisnis organisasi`,
    `${topic} ${audience} Indonesia kebijakan industri perusahaan berita terbaru`,
    `${topic} ${objective} Indonesia perusahaan dampak strategi berita`,
  ];
}
...
export function normalizeIndonesiaNews(batches: Array<{ query: string; results: TavilyResult[] }>) {
  ...
  return Array.from(unique.values())
    ...
    .slice(0, 18)
    .map((source, index) => ({ ref: `IDN${index + 1}`, ...source }));
}
